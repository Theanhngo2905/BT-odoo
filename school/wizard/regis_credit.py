...
class StudentInheritMOdel(models.TransientModel):
    ...
    student_list = fields.Many2one('student' , string='danh sách học sinh')
    subject_list_regis = fields.Many2many(related='student_list.subject_list', string='MÔn học đã đăng ký')
    @api.onchange('student_list')
    def _onchane_subject(self):    
        for r in self:
            if r.student_list:
                r.subject_list_regis = r.student_list.subject_list
